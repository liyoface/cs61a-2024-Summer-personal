...
def balanced(m):
    ...
    # ri = right(m)
    # if is_mobile(end(le)):
    #     if balanced(end(le)) == False:
    #         return False
    # if is_mobile(end(ri)):
    #     if balanced(end(ri)) == False:
    #         return False
    # if total_mass(end(le)) * length(le) == total_mass(end(ri)) * length(ri) :
    #     return True
    # else:
    #     return False
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    spare_stack = 6 - start - end
    if n == 1:
        print_move(start, end)
    else:
        move_stack(n - 1, start, spare_stack)
        print_move(start, end)
        move_stack(n - 1, spare_stack, end)
