...
def deep_map_mut(func, lnk):
    ...
    #     if isinstance(t.first, int):
    #         t.first = func(t.first)
    #     elif isinstance(t.first, Link):
    #         s = t
    #         while not isinstance(s.first, int):
    #             s = s.first
        ...
        while s is not Link.empty:
            if s.first == 'AUG':
                for _ in range(times):
                    s.rest = Link(insert, s.rest)
                    s = s.rest
                break
            else:
                s = s.rest
        insert_gene(lnk.rest, times+1)
    insert_gene(lnk_of_genes, 1)
...
def transcribe(dna):
    ...
    # return [dict[i[0]]+dict[i[1]]+dict[i[2]] for i in [dna[j:j+3] for j in range(0,len(dna),3)]]
