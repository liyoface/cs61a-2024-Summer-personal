...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    # big_num = max(player_score, opponent_score)
    # sma_num = min(player_score, opponent_score)
    # if sma_num == 0:
    #     return False
    # elif big_num % sma_num == 0:
    #     gcd = sma_num
    # else:
    #     while sma_num != 0:
    #         tmp = big_num % sma_num
    ...
    if player_score == 0 or opponent_score ==0:
        ...
        def gcd(current_player, opponent_player):
            if opponent_player == 0:
                return current_player
            else:
                return gcd(opponent_player, current_player%opponent_player)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def say(s0, s1, sh=running_high,last_score=last_score):
        if who == 0:
            if (s0-last_score) > sh:
                sh = s0 - last_score
                print(f'{sh} point(s)! The most yet for Player 0')
            last_score = s0
        else:
            if (s1-last_score) > sh:
                sh = s1 - last_score
                print(f'{sh} point(s)! The most yet for Player 1')
            last_score = s1
        return announce_highest(who,last_score,sh)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average_func(*args):
        ...
        outcomes = outcomes/trials_count
        return outcomes
    return average_func
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_rolls in range(1,11):
        max_score = make_averaged(roll_dice)(num_rolls,dice)
        if  max_score > outcomes:
            outcomes = max_score
            num = num_rolls
    return num
    # END PROBLEM 9
