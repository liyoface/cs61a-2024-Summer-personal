...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        "*** YOUR CODE HERE ***"
        if self.exit:
            self.exit.entrance = self # 有出口的时候就要指定入口Place
        # END Problem 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place is not None and not current_place.is_hive:
            if current_place.bees and (distance <= self.upper_bound) and (distance >= self.lower_bound):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        current_place = self.place  # 保存当前地点的引用
        bees_copy = list(current_place.bees)  # 复制当前地点的蜜蜂列表

        # 对所有蜜蜂造成反射伤害
        for bee in bees_copy:
            bee.reduce_health(amount)

        # 调用父类的 reduce_health 方法减少 FireAnt 的健康值
        super().reduce_health(amount)

        # 如果 FireAnt 死亡，造成额外的伤害
        if self.health <= 0:
            bees_copy = list(current_place.bees)  # 再次复制蜜蜂列表
            for bee in bees_copy:
...
# The WallAnt class
class WallAnt(Ant):
    """An WallAnt that does nothing each turn with large health value."""
...
class TankAnt(ContainerAnt):
    """A ContainerAnt that protects an ant in its place and also deals 1 damage to all bees in its place each turn."""
    name = 'Tank'
    implemented = True
    food_cost = 6
    damage = 1
...
class ScubaThrower(ThrowerAnt):
    """Ant is more costly and waterproof."""
    name = 'Scuba'
    implemented = True
    food_cost = 6
    is_waterproof = True
        ...
        while cur_place is not None:
            if cur_place.ant:
                if cur_place.ant.is_container:
                    if cur_place.ant.ant_contained and not cur_place.ant.ant_contained.doubled:
                        cur_place.ant.ant_contained.double()
                    if not cur_place.ant.doubled:
                        cur_place.ant.double()
                elif not cur_place.ant.doubled:
                    cur_place.ant.double()
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        if target:
            target.slow_duration = 5  # Reset the slow duration
            # Original action method of the Bee
            ...
            def slowed_action(gamestate):
                if target.slow_duration > 0:
                    if gamestate.time % 2 == 0:
                        target.original_action(gamestate)  # Perform the bee's action
                    target.slow_duration -= 1
                else:
                    target.action = target.original_action
                    target.original_action(gamestate)  # Perform the bee's action

            # Assign the slowed action to the bee
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # Special handling for bee direction EC2
