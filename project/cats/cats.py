...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select_para = [i for i in paragraphs if select(i)]
    if k > len(select_para) - 1:
        return ''
    else:
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(paragraph):
        lst = split(remove_punctuation(lower(paragraph)))
        for i in lst:
            if i in subject:
                return True
        else:
...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        ...
        for i in range(len(typed_words)):
            if i <= len(source_words) -1 and typed_words[i] == source_words[i]:
                count += 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    #     return typed_word
    # else:
    #     diff_values = [diff_function(typed_word, i, limit) for i in word_list]
    #     if min(diff_values) > limit:
    #         return typed_word
    #     else:
    #         return word_list[diff_values.index(min(diff_values))]
    if typed_word in word_list:
        return typed_word

        # 计算 typed_word 与 word_list 中每个单词的差异
...
def furry_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    if limit < 0:
        return limit + 1  # Return a value greater than limit
    if len(typed) == 0:
        return len(source)
    if len(source) == 0:
        return len(typed)
    ...
    # Recursive case
    substitute_cost = 1 + furry_fixes(typed[1:], source[1:], limit - 1)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[lst[i+1]-lst[i] for i in range(len(lst)-1)] for lst in timestamps_per_player]
    return words, times
    # END PROBLEM 9
...
def fastest_words(match_object):
    ...
    for i in word_indices:
        scores = [get_time(match_object, j, i) for j in player_indices]
        p_index += [scores.index(min(scores))]
    for player in player_indices:
        wd = [get_word(match_object, i) for i in word_indices if p_index[i] == player ]
        output.append(wd)
    return output
    # END PROBLEM 11
