...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        if self.num >= FreeChecking.free_withdrawals:
            self.balance = self.balance - FreeChecking.withdraw_fee
        self.num += 1
...
class BankAccount:
    ...
    def transactions(self):
        Transaction(self.times, self.balance)
...
class Server:
    ...
    def send(self, email):
        ...
        """
        Client(self).inbox.append(email)
...
class Client:
    ...
    def compose(self, message, recipient_name):
        ...
        email = Email(message, self.server, recipient_name)
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        self.coins[coin] = 1 + self.coins.get(coin, 0) # 提前存入该硬币，如果没有零钱可以换，则result返回自身，由remove_one取出该硬币
